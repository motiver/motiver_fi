...
import com.delect.motiver.client.presenter.training.WorkoutsListPresenter;
import com.delect.motiver.client.presenter.training.WorkoutsListPresenter.WorkoutsListHandler;
...
public class WorkoutsListView extends WorkoutsListPresenter.WorkoutsListDisplay {
	
	private MessageBox box = null;
	private WorkoutsListHandler handler;
  ...
  private ButtonsPanel panelButtons = new ButtonsPanel();
	...
	public WorkoutsListView() {
		...
		this.setStyleName("panel-workouts-list");
		...
		//title
		labelTitle.setText(AppController.Lang.MyWorkouts());
    ...
    if(visible) {
      MyButton btn = panelButtons.addButton(ButtonTarget.Copy, Style.GREEN, AppController.Lang.CopyTo(AppController.Lang.MyWorkouts()));
      btn.addListener(Events.OnClick, new Listener<BaseEvent>() {
        @Override
        public void handleEvent(BaseEvent be) {
	...
	@Override
	public void setHandler(WorkoutsListHandler handler) {
	...
	@Override
	public void setMoveToRoutineButtonVisible(boolean visible, RoutineModel routine) {
    if(visible) {
      String text = (routine != null)? AppController.Lang.MoveTo(routine.getName()) : "";
      MyButton btn = panelButtons.addButton(ButtonTarget.MoveToTarget, Style.GREEN, text);
      btn.addListener(Events.OnClick, new Listener<BaseEvent>() {
        @Override
        public void handleEvent(BaseEvent be) {
          handler.onMoveToRoutineButtonClicked();
        }
      });
    }
    else {
