...
import com.delect.motiver.client.presenter.cardio.RunValueLinkPresenter.RunValueLinkHandler;
...
public class RunValueLinkView extends RunValueLinkPresenter.RunValueLinkDisplay {

	private MessageBox box = null;
	private RunValueLinkHandler handler;

	private LayoutContainer panelButtons = new LayoutContainer();
	private RunValueModel runValue;

	private LayoutContainer thisContent = new LayoutContainer();
	private Timer timerOut;
	
	public RunValueLinkView() {
	...
	public Widget asWidget() {
      ...
      if(runValue.getUid().equals(AppController.User.getUid())) {
        ...
        this.addListener(Events.OnMouseOut, new Listener<BaseEvent>() {
          ...
          public void handleEvent(BaseEvent be) {
			...
			//icon
			Image icon = new Image(MyResources.INSTANCE.iconRun());
			thisContent.add(icon, new HBoxLayoutData(new Margins(0, 20, 0, 0)));			
			
			//name
			if(runValue.getName() != null) {
				Text textName = new Text(runValue.getName().getNameClient() + ": " + CommonUtils.getDurationString( runValue.getDuration()));
	...
	@Override
	public void setHandler(RunValueLinkHandler handler) {
		this.handler = handler;
	}

	@Override
	public void setModel(RunValueModel runValue) {
